import React, {useState} from 'react';
import {
    DesktopOutlined, HeartOutlined,
    HomeOutlined, ImportOutlined, InboxOutlined,
    LaptopOutlined,
    ManOutlined, ShoppingCartOutlined, UnorderedListOutlined, UserOutlined,
    WomanOutlined,
} from '@ant-design/icons';
import {Avatar, Card, Layout, Menu, theme} from 'antd';
import FooterPage from "../FooterPage/FooterPage";
import {Link, Route, Routes} from "react-router-dom";
import Blog from "../Body/Blog/Blog";
import styles from './styles.module.css'
import Home from "../Body/Home/Home";
import Man from "../Body/Man/Man";
import Woman from "../Body/Woman/Woman";
import Technology from "../Body/Technology/Technology";
import Basket from "../Basket/Basket";
import User from "../User/User";
import ProductDetails from "../Body/Product/ProductDetails/ProductDetails";
import MyOrders from "../User/MyOrders/MyOrders";
import UserInformation from "../User/UserInformation/UserInformation";
import Favorites from "../User/Favorites/Favorites";

const {Header, Sider} = Layout;
const { Meta } = Card;

function Navbar(props) {

    const [collapsed, setCollapsed] = useState(false);
    const {
        token: {colorBgContainer},
    } = theme.useToken();

    return (
        <Layout
            style={{
                minHeight: '100vh',
            }}
        >
            <Sider collapsible collapsed={collapsed} onCollapse={(value) => setCollapsed(value)}>
                <div className="demo-logo-vertical"/>
                <img src="https://cdn.logo.com/hotlink-ok/logo-social.png" alt="logo" className={styles.image}/>
                <Menu theme="dark" defaultSelectedKeys={['1']} mode="inline">
                    <Menu.SubMenu key="sub1" title={<span><DesktopOutlined/><span>Navigation One</span></span>}>
                        <Menu.ItemGroup key="g1" title="Item 1">
                            <Menu.Item key="1">Option 1</Menu.Item>
                            <Menu.Item key="2">Option 2</Menu.Item>
                        </Menu.ItemGroup>
                        <Menu.ItemGroup key="g2" title="Item 2">
                            <Menu.Item key="3">Option 3</Menu.Item>
                            <Menu.Item key="4">Option 4</Menu.Item>
                        </Menu.ItemGroup>
                    </Menu.SubMenu>
                </Menu>
            </Sider>
            <Layout>
                <Header
                    style={{
                        padding: 0,
                        background: colorBgContainer,
                    }}
                >
                    <Menu mode="horizontal">
                        <Menu.Item key={'home'} icon={<HomeOutlined/>}>
                            <Link to={'/'}>Home</Link>

                        </Menu.Item>
                        <Menu.Item key={'man'} icon={<ManOutlined/>}>
                            <Link to={'man'}>Man</Link>
                        </Menu.Item>
                        <Menu.Item key={'woman'} icon={<WomanOutlined/>}>
                            <Link to={'woman'}>Woman</Link>
                        </Menu.Item>
                        <Menu.Item key={'technology'} icon={<LaptopOutlined/>}>
                            <Link to={'Technology'}>Technology</Link>
                        </Menu.Item>
                        <Menu.SubMenu key="basket" className={styles.user} title={<Link className={styles.link} to={'basket'}><span><ShoppingCartOutlined /><span>Basket</span></span></Link>}>
                            <Menu.ItemGroup key="group1" title="My Basket">
                                    <Link to={'/details/sdad'}>
                                        <Card style={{ width: 300, marginTop: 16 }} loading={false}>
                                            <Meta
                                                avatar={<Avatar src="https://cdn.dsmcdn.com/ty893/product/media/images/20230520/13/353225973/918706185/1/1_org_zoom.jpg" />}
                                                title='Smart Watch'
                                                description="Size : Default , Piece : 1 "

                                            />
                                        </Card>
                                    </Link>
                                    <Link to={'/details/sdad'}>
                                        <Card style={{ width: 300, marginTop: 16 }} loading={false}>
                                            <Meta
                                                avatar={<Avatar src="data:image/jpeg;base64," />}
                                                title='Headset'
                                                description="Size : Default , Piece : 2 "

                                            />
                                        </Card>
                                    </Link>


                            </Menu.ItemGroup>
                        </Menu.SubMenu>
                        <Menu.SubMenu key="user" className={styles.user}
                                      title={<Link to={'userinformation'} className={styles.link}><span><UserOutlined /><span>User</span></span></Link>}>
                            <Menu.Item key="user1">
                                <Link to={'myorders'}><span><InboxOutlined /><span>My Orders</span></span></Link>
                            </Menu.Item>
                            <Menu.Item key="user2">
                                <Link to={'userinformation'}><span><UnorderedListOutlined /><span>User Information</span></span></Link>
                            </Menu.Item>
                            <Menu.Item key="user3">
                                <Link to={'favorites'}><span><HeartOutlined /><span>My Favorites</span></span></Link>
                            </Menu.Item>
                            <Menu.Item key="user4">
                                <Link to={'logout'}><span><ImportOutlined /><span>Logout</span></span></Link>
                            </Menu.Item>


                        </Menu.SubMenu>


                    </Menu>
                </Header>
                <Routes>
                    <Route path={'/'} element={<Home/>}></Route>
                    <Route path={'footer'} element={<FooterPage/>}></Route>
                    <Route path={'blogs'} element={<Blog/>}></Route>
                    <Route path={'man'} element={<Man/>}></Route>
                    <Route path={'woman'} element={<Woman/>}></Route>
                    <Route path={'technology'} element={<Technology/>}></Route>
                    <Route path={'basket'} element={<Basket/>}></Route>
                    <Route path={'user'} element={<User/>}></Route>
                    <Route path={'details/:id'} element={<ProductDetails/>}></Route>
                    <Route path="myorders" element={<MyOrders />} />
                    <Route path="userinformation" element={<UserInformation />} />
                    <Route path="favorites" element={<Favorites />} />



                </Routes>


            </Layout>
        </Layout>
    );
}

export default Navbar;